...
def compare_dicts(dict1: dict, dict2: dict, dict1_name: str = "d1", dict2_name: str = "d2", path: str = "") -> bool:
    """
    ...
    # recursive comparisons and track what's being compared.
    result = True
    for key in dict1.keys():
        ...
        # Hack for NetBox v2.6.7 requiring integers for some values
            ...
            if dict1[key] != dict2[key]["value"]:
                ...
                result = False
        # Hack for NetBox VCPUs integer as string values like "4.00"
        elif key == "vcpus":
...
def format_slug(text: str) -> str:
    ...
    """
    allowed_chars = (
        string.ascii_letters  # Alphabet
        + string.digits  # Numbers
        + "_-"  # Symbols
    )
    # Replace separators with dash
    separators = " ,."
    replace_separators_with_dash = {ord(sep): ord('-') for sep in separators}
    text = text.translate(replace_separators_with_dash)
...
def format_vcenter_conn(conn: dict) -> dict:
    """
...
def is_banned_asset_tag(text: str) -> bool:
    """
...
async def reverse_lookup(resolver: aiodns.DNSResolver, ip: str) -> tuple:
    """
...
def truncate(text: str = "", max_len: Optional[int] = 50) -> str:
    """
...
def verify_ip(ip_addr: str) -> bool:
    """
...
class VcenterHandler:
    ...
    def __init__(self, vc_conn, nb_api_version):
        ...
        self.tags = [{"name": "Synced"}, {"name": "vCenter"}, format_tag(self.vc_host)]
    ...
    def create_view(self, vc_obj_type: str):
        """
        ...
        return self.vc_session.viewManager.CreateContainerView(
            ...
            True  # Should we recursively look into view
        )

    def get_objects(self, vc_obj_type) -> dict:
        """
        Collects vCenter objects of type and returns NetBox formatted objects.
        ...
        for obj in container_view.view:
            try:
                ...
                if vc_obj_type == "datacenters":
                    ...
                    if obj_manuf_name not in [res["name"] for res in results["manufacturers"]]:
                        log.debug(
                    ...
                    if obj_model not in [res["model"] for res in results["device_types"]]:
                        log.debug(
                    ...
                    results["devices"].append(nbt.device(
                        ...
                        site=settings.NB_SITE,
                        serial=serial_number,
                        asset_tag=asset_tag,
                        cluster=obj.parent.name if obj.parent.name != obj_name else None,
                        status=int(
                            1 if obj.summary.runtime.connectionState == "connected" else 0
                        ),
                        tags=self.tags
                    ))
...
class NetBoxHandler:
    ...
    def __init__(self, vc_conn: dict):
    ...
    def _create_nb_session(self) -> requests.Session:
        """
    ...
    def get_primary_ip(self, nb_obj_type: str, nb_id: int) -> dict:
        """
    ...
    def request(self, req_type: str, nb_obj_type: str, data: Optional[dict] = None,
        ...
        if req_type in ("post", "path"):
            if nb_obj_type == "device_types":
        ...
        if req.status_code == 200:
            ...
            log.warning(
                "Received %s status when attempting to delete NetBox object "
    ...
    def fetch_assignment_id(self, vc_data: dict) -> dict:
        """Fetch assignment object ID for IP address based on assignment dictionary"""
        q_interface = vc_data["assigned_object"]["name"]
        res = False
        if vc_data["assigned_object_type"] == "dcim.interface":
        ...
        if res:
            if res["count"] != 0:
                vc_data["assigned_object_id"] = res["results"][0]["id"]
        return vc_data

    def obj_exists(self, nb_obj_type: str, vc_data: dict):
        """
        ...
        if req["count"] == 1:
            ...
            # NetBox 2.8 changes mitigation
            if "tags" in nb_data:
                nb_data["tags"] = [{"name": tag["name"]} for tag in nb_data["tags"]]
            ...
            if ("tags" in vc_data) and ("Orphaned" in [tag["name"] for tag in nb_data["tags"]]):
                log.info(
                ...
                if "tags" in vc_data:
                    ...
                    log.debug(f"vc_data['tags']: {vc_data['tags']}")
                    tag_names = list(
                        set([tag["name"] for tag in vc_data["tags"]] +
                            [tag["name"] for tag in nb_data["tags"]])
                    )
                    vc_data["tags"] = [{"name": tag} for tag in tag_names]
                ...
                if nb_obj_type == "ip_addresses":
                    ...
                    vc_data = self.fetch_assignment_id(vc_data)
                    if not vc_data:
                        log.warning(
                            "Can't find assignment for the %s object '%s' - canceling update",
                            nb_obj_type, vc_data[query_key]
                        )
                        return
            ...
            if nb_obj_type == "ip_addresses":
                log.debug(
                    "Obtaining NetBox ID of the assigned object to bind it to the %s object",
                    nb_obj_type
                )
                vc_data = self.fetch_assignment_id(vc_data)
                if not vc_data:
                    log.warning(
                        "Can't find assignment for the %s object '%s' - canceling creation",
                        nb_obj_type, vc_data[query_key]
                    )
                    return
            self.request(
    ...
    def sync_objects(self, vc_obj_type: str):
        """
    ...
    def prune_objects(self, vc_objects: dict, vc_obj_type: str):
        """
        ...
        :param vc_objects: Nested dict of extracted vCenter objects sorted by NetBox object type keys
        :param vc_obj_type: vCenter object type to extract, must be key in obj_type_map
        """
        ...
        for nb_obj_type in nb_obj_types:
            ...
            if vc_obj_type == "hosts" and nb_obj_type == "interfaces":
                ...
                for obj in nb_objects:
                    if (obj["assigned_object"] is None) or\
                            ("device" not in obj["assigned_object"]) or (obj["assigned_object"]["device"] is None):
                        continue
                    new_nb_objects.append(obj)
                nb_objects = new_nb_objects
            ...
            elif (vc_obj_type == "virtual_machines") and (nb_obj_type == "ip_addresses"):
                new_nb_objects = []
                for obj in nb_objects:
                    if (obj["assigned_object"] is None) or ("virtual_machine" not in obj["assigned_object"]) or\
                            (obj["assigned_object"]["virtual_machine"] is None):
                        continue
                    new_nb_objects.append(obj)
                nb_objects = new_nb_objects
            ...
            for orphan in orphans:
                ...
                if "Orphaned" not in [o["name"] for o in orphan["tags"]]:
                    log.info(
                        "No tag found. Adding 'Orphaned' tag to %s '%s' object.",
                        nb_obj_type, orphan[query_key]
                    )
                    tags = {
                        "tags": [format_tag("Synced"), format_tag("vCenter"), self.vc_tag, format_tag("Orphaned")]
                    }
                    self.request(
                        req_type="patch", nb_obj_type=nb_obj_type,
                        nb_id=orphan["id"],
    ...
    def search_prefix(self, ip_addr: str) -> dict:
        """
    ...
    def verify_dependencies(self):
        ...
        dependencies = {
            ...
            "sites": [{
                "name": settings.NB_SITE,
                "slug": format_slug(settings.NB_SITE),
            ...
            "tags": [
                {
                    ...
                    "slug": "orphaned",
                    "color": "607d8b",
                    "description": "Applies to objects become orphaned. Source system that earlier provided "
                                   "object no longer states it exists. Object with Orphaned tag will "
                                   "remain in this state until it ages and is automatically removed"
                    if settings.NB_PRUNE_ENABLED else ""
                },
                {
                    "name": self.vc_tag["name"],
                    "slug": format_slug(self.vc_tag["name"]),
                    "description": "Objects synced from vCenter host "
                                   "{}. Be careful not to modify the name or "
                                   "slug.".format(self.vc_tag["name"])
                }]
        }
